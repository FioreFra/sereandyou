(function (global) {
  ...
  /* ═══════════════════════════════════════════════════════════
     CONFIGURAZIONE — incolla qui i tre link CSV pubblicati da
     Google Sheets:
       Google Sheets → File → Condividi → Pubblica sul web
       → scegli il singolo foglio ("Post", "Outfit" o "Prodotti")
       → formato CSV → Pubblica → copia il link.
     Finché uno dei tre resta col valore placeholder, il sito
     continua a funzionare con i dati di esempio qui sotto.
     ═══════════════════════════════════════════════════════════ */
  const CONFIG = {
    postsCsvUrl: 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRUT01FCJOHHyw9j5jgyJZT0imyx_BXlzdk3fStuAbtnKgB4L96JV1NO8Ujavx4yX0IrSRWWys4k2gF/pub?gid=1426389611&single=true&output=csv',
  ...
  const SAMPLE_LOOKS = {
    l1: {
      ...
      link_carosello: '#', prodotti_id: ['p01', 'p02', 'p03'], post_id: 'post1',
    },
    l2: {
      ...
      link_carosello: '#', prodotti_id: ['p04', 'p05'], post_id: 'post1',
    },
    l3: {
      ...
      link_carosello: '#', prodotti_id: ['p05', 'p06'], post_id: 'post2',
    },
  };

  const SAMPLE_POSTS = {
    post1: {
      nome: 'Carosello "Da ufficio ad aperitivo"', immagini: [],
      caption: 'Due look, una sola giornata: come passo dall\'ufficio all\'aperitivo senza tornare a casa a cambiarmi.',
    },
    post2: {
      nome: 'Carosello "Weekend fuori porta"', immagini: [],
      caption: 'Il look che metto quando so che camminerò tutto il giorno.',
    },
  };

  function isConfigured() {
    return /^https?:\/\//.test(CONFIG.postsCsvUrl) && /^https?:\/\//.test(CONFIG.outfitsCsvUrl) && /^https?:\/\//.test(CONFIG.productsCsvUrl);
  }
  ...
  async function loadData() {
    if (!isConfigured()) {
      return { products: SAMPLE_PRODUCTS, looks: SAMPLE_LOOKS, posts: SAMPLE_POSTS, usingFallback: true };
    }
    try {
      const [productRows, outfitRows, postRows] = await Promise.all([
        parseCsv(CONFIG.productsCsvUrl),
        parseCsv(CONFIG.outfitsCsvUrl),
        parseCsv(CONFIG.postsCsvUrl),
      ]);
      ...
      return { products, looks, posts, usingFallback: false };
    } catch (err) {
  ...
  function looksForPost(looks, postId) {
    return Object.keys(looks)
      .filter((id) => looks[id].post_id === postId)
      .reduce((acc, id) => { acc[id] = looks[id]; return acc; }, {});
  }

  global.SereYouData = { loadData, imageList, formatPrice, looksForPost };
})(window);
